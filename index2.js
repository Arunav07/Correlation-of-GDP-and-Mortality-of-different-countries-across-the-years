...
async function init() {
    var worldData = await d3.csv("./World_Data.csv")
    processDataSet(worldData, 0)//Page 1
    mortalityData = await d3.csv("./Mortality_rate.csv")
...
function showScene(dataSet) {
    var Annotations = document.getElementById("notes");
    var commonText = "<br> To get the exact data values, move the mouse over each data point to show tooltip with details"
    if (sceneIndex == 0) {
        Annotations.innerHTML = "<b>World GDP per Capita:</b> Notice a sudden dip in world gdp in 2008-2009 due to <i>Great Recession<i/> and another dip in 2020 due to the <i>Covid-19 Pandemic</i>.<br><i>" +commonText +"</i>"
    }else if(sceneIndex == 1){
        Annotations.innerHTML = "<b>World Mortality rate, infant (per 1,000 live births):</b> Year <i>2002-2003</i> shows the largest decline in mortality rate, however a sharp increase is noticed during 2020 due to the <i>Covid-19 Pandemic</i>.<br><i>" +commonText +"</i>"
    }else if(sceneIndex == 2){
        flag = dataSet.filter(myFunction);
        if(flag.length == 0){
            if(hoverIndex == 0){
                Annotations.innerHTML = "<p style=\"color:red\"> GDP data for <b>"+selectedText+"</b> is not available </p> @ <a href=\"https://data.worldbank.org/indicator/NY.GDP.PCAP.PP.CD\" target=\"_blank\">GDP per capita, PPP</a>"
            }else{
                Annotations.innerHTML = "<p style=\"color:red\"> Mortality data for <b>"+selectedText+"</b> is not available </p> @ <a href=\"https://data.worldbank.org/indicator/SP.DYN.IMRT.IN\" target=\"_blank\">Mortality rate, infant</a>"
            }    
        } else {
            if(hoverIndex == 0){
                Annotations.innerHTML = "GDP data for <b>"+selectedText+"</b> <br><i>" +commonText +"</i>"
            }else{
                Annotations.innerHTML = "Mortality data for <b>"+selectedText+"</b> <br><i>" +commonText +"</i>"
            }
        }
    }
    ...
    if(hoverIndex == 0)
    {
        ...
        Graph_Title ="GDP per capita vs Year"
    }
    else
    {
        dot_color="dot_blue";
        line_color = "line_blue";
        Graph_Title ="Change in Mortality per Year"
    }
    ...
    if (sceneIndex != 2) {
        showAnnotation(sceneIndex,  xScale, yScale)
    }
}

function showAnnotation(sceneIndex,  xAxis, yAxis) {
    parent = d3.select("#chartBoxId")
        .select("svg")
        .append("g")

if(isFinalScene==false){
    if (sceneIndex == 0) {
        parent.append("rect")
            .attr("x", xAxis(2009) - 280)
            .attr("y", yAxis(17024.25) + 20 )
            .attr("fill", "lightgrey")
            .attr("class", "annotationBox")

        parent.append("text")
            .attr("x", xAxis(2009) - 230)
            .attr("y", yAxis(17024.25) + 50)
        ...
        parent.append("rect")
            .attr("x", xAxis(2020) - 90)
            .attr("y", yAxis(29.046) - 130)
            .attr("fill", "lightgrey")
            .attr("class", "annotationBox1")

        parent.append("text")
            .attr("class", "annotation_text")
            .attr("x", xAxis(2020) - 80)
            .attr("y", yAxis(29.046) - 95)
            .text("COVID-19");
...
function moveToScene(location) {
    ...
    if(location=="Correlation"){
        sceneIndex = 2;
        hoverIndex=2;
        isGDP = false;
        showScene(sceneData[2])
            d3.select(".scene3box")
                .style("visibility", "hidden")
    }
...
function removeFinalScene(){
    d3.select(".scene3box").style("visibility", "hidden");
    isFinalScene = false;
    moveToScene(isGDP?"GDP":"Mortality");
}
